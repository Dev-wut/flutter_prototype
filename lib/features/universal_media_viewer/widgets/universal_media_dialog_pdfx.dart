...
class UniversalMediaDialogPdfx extends StatefulWidget {
  const UniversalMediaDialogPdfx({
    super.key,
  ...
  static Future<void> showSingle({
    ...
    await showDialog(
      ...
      builder: (context) => Dialog.fullscreen(
        child: UniversalMediaDialogPdfx(
  ...
  @override
  State<UniversalMediaDialogPdfx> createState() => _UniversalMediaDialogPdfxState();
}

class _UniversalMediaDialogPdfxState extends State<UniversalMediaDialogPdfx> {
  ...
  void dispose() {
    ...
    // Close PDF documents (ไม่ต้อง dispose controllers)
  ...
  Future<void> _cleanupTempFiles() async {
    try {
      ...
      // Clear controllers map (ไม่ต้อง dispose เพราะสร้างใหม่ทุกครั้ง)
  ...
  Future<void> _preloadCurrentFile() async {
    ...
    if (currentItem.type == MediaType.pdf) {
      await _loadPDFWithRetry(_currentIndex);
    }
  }

  Future<void> _loadPDF(int index) async {
    if (_downloadingStates[index] == true || _pdfDocuments[index] != null) return;
    ...
    try {
      ...
      if (currentItem.hasLocalData) {
        ...
        document = await PdfDocument.openData(currentItem.localData!);
      } else {
        // Load from network
      ...
      // Create controller ใหม่ทุกครั้ง
      setState(() {
        _pdfDocuments[index] = document;
        _pdfControllers[index] = null; // จะสร้างใหม่ใน build
  ...
  void _onPageChanged(int index) {
    ...
    final currentItem = widget.mediaItems[index];
    if (currentItem.type == MediaType.pdf && _pdfDocuments[index] == null) {
      _loadPDFWithRetry(index);
    }
  ...
  Widget _buildCurrentMedia() {
    ...
    switch (currentItem.type) {
      case MediaType.pdf:
        final pdfDocument = _pdfDocuments[_currentIndex];
        if (pdfDocument != null) {
          // สร้าง controller ใหม่ทุกครั้งเพื่อหลีกเลี่ยง Future error
          return Container(
  ...
  Future<void> _loadPDFWithRetry(int index, {int maxRetries = 3}) async {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text(
              currentItem.title ?? 'Media Viewer (PDFx)',
...
extension BuildContextExtensionPdfx on BuildContext {
  Future<void> showMediaDialogPdfx({
    required String url,
    MediaType? type,
    String? title,
    Uint8List? localData,
  }) async {
    await UniversalMediaDialogPdfx.showSingle(
      context: this,
      url: url,
      type: type ?? UniversalMediaDialogPdfx._detectMediaType(url),
      title: title,
      localData: localData,
    );
  }

  Future<void> showMultiplePdfx({
    required List<MediaItemModel> mediaItems,
    int initialIndex = 0,
    String? title,
  }) async {
    await UniversalMediaDialogPdfx.showMultiple(
      context: this,
      mediaItems: mediaItems,
      initialIndex: initialIndex,
      title: title,
  ...
  Future<void> showMediaGalleryPdfx({
    required List<String> urls,
    List<String>? titles,
    int initialIndex = 0,
    String? title,
  }) async {
    await UniversalMediaDialogPdfx.showFromUrls(
      context: this,
      urls: urls,
      titles: titles,
      initialIndex: initialIndex,
